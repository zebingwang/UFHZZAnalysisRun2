...
infile = TFile('Sync_102X.root','READ')
#infile = TFile('Sync_102X_test.root','READ')
ext=''
#ext='_shortest'
#ext='_70to110'
#ext='_150to500'
...
with open('eventsBUF_SyncM19_v3'+ext+'.txt','w') as f:
  ...
  for i in xrange(nevents):
    ...
    #if (not (t.finalState==2)): continue
    ...
    if (ext=='_shortest'):
      ...
      f.write('{}:{}:{}:{:.2f}:{:.2f}:{:.2f}:{:.3f}:{:.3f}:{}:{:.2f}:{:.2f}:{:.3f}:{:.3f}:{:.3f}:{:.3f}:{:.3f}:{:.3f}:{:.3f}:{}:{:.3f}'.format(t.Run,t.LumiSect,t.Event,t.mass4l,t.massZ1,t.massZ2,t.D_bkg_kin,t.D_bkg,t.njets_pt30_eta4p7,ptjet1,ptjet2,t.D_bkg_VBFdec,t.D_bkg_VHdec,t.D_VBF,t.D_VBF1j,t.D_HadWH,t.D_HadZH,t.met,0,t.pileupWeight*t.genWeight)+'\n')
